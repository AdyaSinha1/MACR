...
def temp_faiss_store(tmp_path):
    ...
    store = FaissMemoryStore(embedding_service, index_file=index_file, meta_file=meta_file)
    return store

def test_store_and_retrieve_review(temp_faiss_store):
    f1 = Finding(agent_name="A", category="bug", severity="high", code_location="x:1", description="desc", explanation="exp", confidence=1.0)
    report = FinalReport(file_path="main.py", findings=[f1], agent_agreement=1.0, total_confidence=1.0)

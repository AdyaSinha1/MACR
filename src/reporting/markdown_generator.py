...
def generate_markdown_report(report: FinalReport) -> str:
    ...
    lines = []
    lines.append(f"# Multi-Agent Code Review Report")
    ...
    lines.append(f"**Overall Confidence**: {round(report.total_confidence * 100, 1)}%\n")
    ...
    for i, finding in enumerate(report.findings, 1):
        ...
        lines.append(f"**Severity**: `{finding.severity}` | **Location**: `{finding.code_location}`")
        lines.append(f"**Agent**: `{finding.agent_name}` | **Confidence**: `{round(finding.confidence, 2)}`\n")

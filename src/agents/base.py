...
class BaseAgent(abc.ABC):
    ...
    async def analyze(self, context: SharedContext) -> Finding:
        ...
        if not self.circuit_breaker.is_allowed():
            logger.warning(f"Circuit breaker is open for {self.name}. Skipping analysis.")
            EventBus.publish_sync('agent_error', {'agent': self.name, 'error': 'Circuit breaker open'})
        ...
        try:
            ...
            EventBus.publish_sync('agent_status', {'agent': self.name, 'status': 'starting'})
            finding = await self._run_evaluator_optimizer(context)
            EventBus.publish_sync('agent_status', {'agent': self.name, 'status': 'complete'})
    ...
    async def _run_evaluator_optimizer(self, context: SharedContext) -> Finding:
        ...
        for iteration in range(1, self.max_iterations + 1):
            logger.info(f"{self.name} loop iteration {iteration}", current_confidence=finding.confidence)
            EventBus.publish_sync('agent_progress', {'agent': self.name, 'iteration': iteration, 'confidence': finding.confidence})
            
            # Stop if confidence is high enough
            if finding.confidence > 0.85:
                logger.info(f"{self.name} reached sufficient confidence ({finding.confidence}). Stopping refinement.")
                break
                
            # 2. Evaluator Step (Self-Critique)
            critique_prompt = self.get_critique_prompt(finding)
            critique = await self.llm.generate_structured(critique_prompt, AgentCritique)
    ...
    def _create_error_finding(self, error_message: str) -> Finding:
        ...
        derived_category = "style" if "style" in self.name.lower() else ("security" if "security" in self.name.lower() else "bug")
        return Finding(

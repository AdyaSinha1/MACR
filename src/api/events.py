...
class EventBus:
    ...
    @classmethod
    async def publish(cls, event_type: str, data: Any, job_id: str = None):
        target_job_id = job_id or active_job_id.get()
        if target_job_id and target_job_id in cls._queues:
            await cls._queues[target_job_id].put({"event": event_type, "data": data})

    @classmethod
    def publish_sync(cls, event_type: str, data: Any, job_id: str = None):
        target_job_id = job_id or active_job_id.get()
        if target_job_id and target_job_id in cls._queues:
            try:

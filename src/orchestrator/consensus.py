...
class ConsensusEngine:
        ...
        for group in groups:
            if len(group) == 1:
                ...
                try:
                    ...
                    resolved.merged_finding.explanation += f"\n\n[Consensus Rationale]: {resolved.resolution_rationale}"
                    resolved_findings.append(resolved.merged_finding)
                except Exception as e:
                    logger.error("Failed to resolve conflict, falling back to all findings", error=str(e))
                    resolved_findings.extend(group) # Rule-based fallback: just keep them all on failure
                
        # Calculate metrics
        avg_confidence = sum(f.confidence for f in resolved_findings) / len(resolved_findings) if resolved_findings else 1.0
        
        # Agreement heuristic: high if many findings collapsed into few.
        # (Initial Findings - Final Findings) / Initial Findings
        agreement_ratio = (len(all_findings) - len(resolved_findings)) / len(all_findings) if len(all_findings) > 0 else 1.0
    ...
    def _group_by_location(self, findings: List[Finding], tolerance: int = 5) -> List[List[Finding]]:
        ...
        for finding in findings:
            ...
            for group in groups:
                ...
                if self._overlap(finding.code_location, group[0].code_location, tolerance):
